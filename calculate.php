 ...
 <table  style="border:1px solid #acacac">
    <thead>
           <tr style="background: whitesmoke;">
				   ...
				   <th width="15%"><span class="dotvalue"></span>bangladesh</th> 
				   <th width="15%"><span class="dotvalue"></span>biggan</th>
				   <th width="15%"><span class="dotvalue"></span>dharma</th> 
				    <th width="15%"><span class="dotvalue"></span>krisi</th>   
...
//echo "$table";
 ...
 for($i=0; $i<$_POST['numbers']; $i++)
 {
 	...
 	if ($bangp==0||
 		...
 		$_POST[$col8][$i]==0||
 		$_POST[$col9][$i]==0) {
 		$s="UPDATE $table SET totalg='F' ,totalgp=0,
 		ban12gp=(CASE WHEN $ban>=80 AND $ban<=100 THEN 5
		...
		$s="UPDATE result_stt set result_stt=1 WHERE class_n='$table'";
        mysqli_query($conn ,$s);
 	...
 	else
 	{
 		...
 		$s="UPDATE $table SET totalg='$totalg',Totalgp=$total,
 		ban12gp=(CASE WHEN $ban>=80 AND $ban<=100 THEN 5
                               WHEN $ban>=70 AND  $ban<=79 THEN 4
                               WHEN $ban>=60 AND  $ban<=69 THEN 3.5
                               WHEN $ban>=50 AND  $ban<=59 THEN 3
		...
		/* $col3=$_POST['sub']."t";
		 ...
		 $col5=$_POST['sub']."gp";*/
				 ...
				 $sql="SELECT COUNT(id) AS sum FROM ".$_POST['class_name']."  ";
				  ...
				  $query=" SELECT student_id,roll,student_name,$col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9  FROM student_info as info,".$table." as class WHERE info.student_id=class.id ORDER BY roll";
		    ...
		    <input type="hidden" name="bisgp[]" value="<?php echo $row[$col6];?>" />
		    <input type="hidden" name="biggp[]" value="<?php echo $row[$col7];?>" />
		    <input type="hidden" name="dhargp[]" value="<?php echo $row[$col8];?>" />
		    <input type="hidden" name="krigp[]" value="<?php echo $row[$col9];?>" />
