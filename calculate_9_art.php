 ...
 <table  style="border:1px solid #acacac">
    <thead>
           <tr style="background: whitesmoke;">
				   ...
				   <th width="15%"><span class="dotvalue"></span>Science</th> 
				    <th width="15%"><span class="dotvalue"></span>pouroniti</th>
				    <th width="15%"><span class="dotvalue"></span>vugol</th>
				    <th width="15%"><span class="dotvalue"></span>itihas</th> 
...
//echo "$table";
 ...
 for($i=0; $i<$_POST['numbers']; $i++)
 {
 	...
 	if ($bangp==0||
		...
		$s="UPDATE result_stt_art set result_stt_art=1 WHERE class_n='$table'";
		echo "$s";
if(!mysqli_query($conn ,$s))
		 echo mysql_error();
	  else
 	...
 	{  
 		...
 		$total=$total/9;
 		$to_with_out_forth=(($total*9+2)-$_POST[$col11][$i])/9;
		...
		$s="UPDATE result_stt_art set result_stt=1 WHERE class_n='$table'";
if(!mysqli_query($conn ,$s))
		 echo mysql_error();
	  else
		   echo '<script> swal("Marks  Input Update Successful", "!", "success"); </script>';
		...
		/* $col3=$_POST['sub']."t";
		 ...
		 $col5=$_POST['sub']."gp";*/
				 ...
				 $sql="SELECT COUNT(id) AS sum FROM ".$_POST['class_name']."  ";
				  ...
				  $query=" SELECT student_id,class.roll,student_name,$col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12  FROM student_info as info,".$table." as class,".$table2." as some  WHERE info.student_id=class.id AND some.id=class.id ORDER BY class.roll";
		    ...
		    <input type="hidden" name="scigp[]" value="<?php echo $row[$col8];?>" />
		    <input type="hidden" name="pougp[]" value="<?php echo $row[$col9];?>" />
		    <input type="hidden" name="vgolgp[]" value="<?php echo $row[$col10];?>" />
		    <input type="hidden" name="itigp[]" value="<?php echo $row[$col11];?>" />
		    <input type="hidden" name="forthgp[]" value="<?php echo $row[$col12];?>" />
			...
			<input type="hidden" name="col12" value="<?php echo $col12 ; ?>" />
